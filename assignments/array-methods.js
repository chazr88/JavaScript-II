...
runners.forEach(function(runner){
    fullName.push(`${runner.first_name}, ${runner.last_name}`);//IF THIS IS CORRECT I CANT BELIEVE I FIGURED IT OUT ON THE FIRST TRY!!!!!
})
...
let allCaps = [];
const makeThemUpper = runners.map(function(runner){
...
let largeShirts = [];
const shirtSizeFilter = runners.filter(function(runner){
...
const totalDon = runners.reduce((total, don) => {
    return total += don.donation;
}, 0);

ticketPriceTotal.push(totalDon)
...
// Problem 1
let allLower = [];
const makeThemLower = runners.map(function(runner){
    allLower.push(runner.first_name.toLowerCase())
})
...
let introduction = [];
runners.forEach(function(runner){
    introduction.push(`Hello, I am ${runner.first_name} ${runner.last_name} of Company ${runner.company_name}`);//IF THIS IS CORRECT I CANT BELIEVE I FIGURED IT OUT ON THE FIRST TRY!!!!!
})
console.log(introduction);

// Problem 3
let topDonations = [];
const hadTopDonations = runners.filter(function(runner){
    if(runner.donation >= 100){
        topDonations.push(runner)
    }
})
